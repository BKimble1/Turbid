...
enum CameraError: Error, Equatable, Sendable {
    ...
    var message: String {
        switch self {
        case .notAuthorized:
            return "Turbid does not have permission to use the camera."
    ...
    var recoverySuggestion: String? {
        switch self {
        case .notAuthorized:
            return "Turn Camera on for Turbid in Settings."
        case .noSuitableCamera:
            return "Turbid needs a rear camera with a torch and lockable focus, exposure and white balance."
        ...
        case .sessionRuntimeError, .configurationFailed, .cannotAddInput, .cannotAddOutput:
            return "Close and reopen Turbid, then try again."
        case .sessionNotConfigured:
            return nil
        }
    }
