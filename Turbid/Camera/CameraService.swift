...
final class CameraService: NSObject, CameraControlling, @unchecked Sendable {
    ...
    private let sessionQueue = DispatchQueue(label: "com.turbid.camera.session")
    private let processingQueue = DispatchQueue(label: "com.turbid.camera.processing",
                                                qos: .userInitiated)
    ...
    func setTorch(on: Bool) async throws -> TorchStatus {
        try await onSessionQueue { [self] in
            ...
            if on && !status.deliveredRequestedLevel {
                TurbidLog.camera.notice("Torch active but below the requested level; the device is limiting output.")
            }
            return status
        }
    }
    ...
    private func turnTorchOffIgnoringErrors() {
        ...
        do {
            ...
            TurbidLog.camera.error("Could not turn the torch off: \(String(describing: error), privacy: .public)")
        }
        torchStatus = .off
    }
    ...
    func warmUp() async throws -> Bool {
        ...
        if !settled {
            TurbidLog.camera.notice("Camera controls did not settle within the warm-up window.")
        }
        return settled
    }
    ...
    private func installObservers(for device: AVCaptureDevice) {
        ...
        observers.append(center.addObserver(
            ...
            self?.sessionQueue.async { [weak self] in
                ...
                TurbidLog.camera.error("Capture runtime error: \(description, privacy: .public)")
                turnTorchOffIgnoringErrors()
        ...
        pressureObservation = device.observe(\.systemPressureState, options: [.new, .initial]) { [weak self] device, _ in
            ...
            self?.sessionQueue.async { [weak self] in
                ...
                if !level.permitsMeasurement {
                    TurbidLog.camera.notice("System pressure \(level.rawValue, privacy: .public); capture is being throttled.")
                }
                publishSnapshot()
            }
        }
    ...
    private static func describeInterruption(_ rawValue: Int?) -> String {
        ...
        case .videoDeviceNotAvailableInBackground:
            return "the camera is not available while Turbid is in the background"
