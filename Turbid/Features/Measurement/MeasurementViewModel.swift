...
final class MeasurementViewModel {
    ...
    func refreshAuthorization() async {
        ...
        if !status.allowsCapture && machine.state.usesCaptureHardware {
            ...
            TurbidLog.permission.notice("Camera access revoked while a session was active.")
        }
    }
    ...
    func startSetup() async {
        ...
        do {
            ...
            if !torch.isActive {
                TurbidLog.camera.notice("Torch did not come on for alignment.")
            }
    ...
    func interruptCapture(reason: MeasurementInterruption) async {
        ...
        TurbidLog.measurement.notice("Capture shutdown: \(reason.rawValue, privacy: .public)")
        await shutdownCapture()
    }
    ...
    private func runMeasurementSequence() async {
        ...
        do {
            ...
            if !torch.deliveredRequestedLevel {
                TurbidLog.camera.notice("Torch is active below the requested maximum level.")
            }

            let settled = try await environment.camera.warmUp()
            if !settled {
                TurbidLog.camera.notice("Camera never stopped adjusting during warm-up.")
            }
    ...
    private func handleCameraFailure(_ error: CameraError, whilePreparing: Bool) async {
        TurbidLog.camera.error("Camera failure: \(error.message, privacy: .public)")
