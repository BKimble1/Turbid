...
enum CalibrationStoreError: Error, Equatable, Sendable {
    /// The file was written by a newer version of Lucid.
    ...
    var message: String {
        ...
        case .unsupportedSchema(let found, let supported):
            return "These calibrations were saved by a newer version of Lucid (format \(found), this version reads \(supported)). Update Lucid to use them."
...
struct FileCalibrationStore: CalibrationStoring {
    ...
    static func defaultURL(fileManager: FileManager = .default) throws -> URL {
        let directory = try fileManager.url(for: .applicationSupportDirectory,
                                            ...
                                            appropriateFor: nil,
                                            create: true)
        let folder = directory.appendingPathComponent("Lucid", isDirectory: true)
